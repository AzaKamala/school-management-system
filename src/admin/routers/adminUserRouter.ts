...
router.post('/', createAdminUserValidator, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const adminUser = await createAdminUser(email, password, firstName, lastName, role);
...
router.put('/', updateAdminUserValidator, async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        ...
        if (existingAdminUserEmail && existingAdminUserEmail.email !== adminUser.email) {
            res.status(400).json({ error: 'Email already exists!' });
            return;
        }

        const updatedAdminUser = await updateAdminUser(id, email, password, firstName, lastName, role, active);
